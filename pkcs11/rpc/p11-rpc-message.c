...
P11RpcMessage*
p11_rpc_message_new (GkrBufferAllocator allocator)
{
	P11RpcMessage *msg;
	
	assert (allocator);
	
	msg = (P11RpcMessage*) (allocator)(NULL, sizeof (P11RpcMessage));
	if (!msg)
		return NULL;
	...
	p11_rpc_message_reset (msg);
	
	return msg;
}

void 
p11_rpc_message_free (P11RpcMessage *msg)
{
...
void 
p11_rpc_message_reset (P11RpcMessage *msg)
{
	assert (msg);
...
int
p11_rpc_message_prep (P11RpcMessage *msg, int call_id, P11RpcMessageType type)
{
	int len;
...
int 
p11_rpc_message_parse (P11RpcMessage *msg, P11RpcMessageType type)
{
	...
	if (!gkr_buffer_get_uint32 (&msg->buffer, msg->parsed, &(msg->parsed), &call_id)) {
		p11_rpc_warn ("invalid message: couldn't read call identifier");
		return 0;
	}
	...
	if (call_id == P11_RPC_CALL_ERROR) {
		if (type == P11_RPC_REQUEST) {
			p11_rpc_warn ("invalid message: error code in request");
			return 0;
		}
	...
	if (call_id <= 0 || call_id >= P11_RPC_CALL_MAX) {
		p11_rpc_warn ("invalid message: bad call id: %d", call_id);
		return 0;
	}
	if (type == P11_RPC_REQUEST) 
		msg->signature = p11_rpc_calls[call_id].request;
	...
	if (!gkr_buffer_get_byte_array (&msg->buffer, msg->parsed, &(msg->parsed), &val, &len)) {
		p11_rpc_warn ("invalid message: couldn't read signature");
		return 0;
	}
	
	if ((strlen (msg->signature) != len) || (memcmp (val, msg->signature, len) != 0)) {
		p11_rpc_warn ("invalid message: signature doesn't match");
		return 0;
	}
...
int
p11_rpc_message_equals (P11RpcMessage *m1, P11RpcMessage *m2)
{
	assert (m1 && m2);
...
int 
p11_rpc_message_verify_part (P11RpcMessage *msg, const char* part)
{
	int len, ok;
...
int
p11_rpc_message_write_attribute_buffer (P11RpcMessage *msg, CK_ATTRIBUTE_PTR arr, 
                                        CK_ULONG num)
{
...
int
p11_rpc_message_write_attribute_array (P11RpcMessage *msg, 
...
int
p11_rpc_message_read_byte (P11RpcMessage *msg, CK_BYTE *val)
{
	assert (msg);
...
int
p11_rpc_message_write_byte (P11RpcMessage *msg, CK_BYTE val)
{
	assert (msg);
...
int
p11_rpc_message_read_ulong (P11RpcMessage *msg, CK_ULONG *val)
{
	uint32_t v;
	assert (msg);
...
int
p11_rpc_message_write_ulong (P11RpcMessage *msg, CK_ULONG val)
{
	assert (msg);
...
int
p11_rpc_message_write_byte_buffer (P11RpcMessage *msg, CK_ULONG count)
{
	assert (msg);
...
int
p11_rpc_message_write_byte_array (P11RpcMessage *msg, CK_BYTE_PTR arr, CK_ULONG num)
{
	assert (msg);
	assert (!num || arr);
...
int
p11_rpc_message_write_ulong_buffer (P11RpcMessage *msg, CK_ULONG count)
{
	assert (msg);
...
int
p11_rpc_message_write_ulong_array (P11RpcMessage *msg, CK_ULONG_PTR array, CK_ULONG n_array)
{
	CK_ULONG i;
...
int
p11_rpc_message_read_version (P11RpcMessage *msg, CK_VERSION* version)
{
	assert (msg);
	assert (version);
...
int
p11_rpc_message_write_version (P11RpcMessage *msg, CK_VERSION* version)
{
	assert (msg);
	assert (version);
...
int
p11_rpc_message_read_space_string (P11RpcMessage *msg, CK_UTF8CHAR* buffer, CK_ULONG length)
{
	...
	if (n_data != length) {
		p11_rpc_warn ("invalid length space padded string received: %d != %d", length, n_data);
		return 0;
	}
...
int
p11_rpc_message_write_space_string (P11RpcMessage *msg, CK_UTF8CHAR* buffer, CK_ULONG length)
{
	assert (msg);
	assert (buffer);
	assert (length);
