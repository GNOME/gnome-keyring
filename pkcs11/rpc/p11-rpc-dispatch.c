...
/* p11-rpc-dispatch.h - receiver of our PKCS#11 protocol.
...
call_init (CallState *cs)
{
	...
	cs->req = p11_rpc_message_new ((GkrBufferAllocator)realloc);
	cs->resp = p11_rpc_message_new ((GkrBufferAllocator)realloc);
	if (!cs->req || !cs->resp) {
...
#define IN_BYTE(val) \
	if (!p11_rpc_message_read_byte (cs->req, &val)) \
		{ _ret = PARSE_ERROR; goto _cleanup; }

#define IN_ULONG(val) \
	if (!p11_rpc_message_read_ulong (cs->req, &val)) \
		{ _ret = PARSE_ERROR; goto _cleanup; }
...
#define OUT_ULONG(val) \
	if (_ret == CKR_OK && !p11_rpc_message_write_ulong (cs->resp, val)) \
		_ret = PREP_ERROR;
...
dispatch_call (CallState *cs)
{
	...
	assert (req->call_id > P11_RPC_CALL_ERROR);
	assert (req->call_id < P11_RPC_CALL_MAX);
	
	/* Prepare a response for the function to fill in */
	if (!p11_rpc_message_prep (resp, req->call_id, P11_RPC_RESPONSE)) {
		p11_rpc_warn ("couldn't prepare message");
		return 0;
	}
	...
	if (ret == CKR_OK) {
		...
		/* Parsing errors? */
		if (p11_rpc_message_buffer_error (req)) {
			p11_rpc_warn ("invalid request from module, probably too short");
			ret = PARSE_ERROR;
		}

		/* Out of memory errors? */
		if (p11_rpc_message_buffer_error (resp)) {
			p11_rpc_warn ("out of memory error putting together message");
			ret = PREP_ERROR;
		}
	}
	...
	if (ret == CKR_OK) {
		...
		assert (p11_rpc_message_is_verified (resp));
		assert (resp->call_type == P11_RPC_RESPONSE);
		assert (resp->call_id == req->call_id);
		assert (p11_rpc_calls[resp->call_id].response);
		assert (strcmp (p11_rpc_calls[resp->call_id].response, 
		                resp->signature) == 0);
	...
	while (len > 0) {
		...
		if (r == 0) {
			...
			if (errno != EAGAIN && errno != EINTR) {
				p11_rpc_warn ("couldn't receive data: %s", strerror (errno));
				return 0;
			}
		} else {
			data += r;
	...
	while (len > 0) {
		...
		if (r == -1) {
			if (errno == EPIPE) {
				...
				p11_rpc_warn ("couldn't send data: %s", strerror (errno));
				return 0;
			}
		} else {
			data += r;
...
run_dispatch_loop (int sock)
{
	...
	if (!p11_rpc_read_credentials (sock))
		return;
	...
	while (TRUE) {
		...
		if (len >= 0x0FFFFFFF) { 
			p11_rpc_warn ("invalid message size from module: %u bytes", len); 
			break;
		}
		...
		if (gkr_buffer_has_error (&cs.req->buffer)) {
			p11_rpc_warn ("error allocating buffer for message");
			break;
		}
		...
		if (!p11_rpc_message_parse (cs.req, P11_RPC_REQUEST))
			break;
...
p11_rpc_dispatch_accept (void)
{
	...
	if (new_fd < 0) {
		p11_rpc_warn ("cannot accept pkcs11 connection: %s", strerror (errno));
		return;
	}
...
int 
p11_rpc_dispatch_fd (void)
{
	return pkcs11_socket;
}

int
p11_rpc_dispatch_init (const char *socket_path, CK_FUNCTION_LIST_PTR module, 
