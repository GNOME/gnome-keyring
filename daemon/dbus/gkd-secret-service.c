				...
				GkdSecretService *self)
{
	...
	if (n_data > 0) {
		...
		items = gck_objects_from_handle_array (self->internal_session, data, n_data / sizeof (CK_OBJECT_HANDLE));
		...
		item = gck_session_create_object (self->internal_session, gck_builder_end (&builder), NULL, &error);
		if (item == NULL) {
			g_dbus_method_invocation_take_error (invocation, error);
			return TRUE;
		}
