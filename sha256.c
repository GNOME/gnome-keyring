...
static const guint32 hinit[8] = {
	0x6a09e667U, 0xbb67ae85U, 0x3c6ef372U, 0xa54ff53aU, 0x510e527fU, 0x9b05688cU, 0x1f83d9abU, 0x5be0cd19U
};

int sha256Reset(register sha256Param* sp)
{
	memcpy(sp->h, hinit, 8 * sizeof(guint32));
	memset(sp->data, 0, 64 * sizeof(guint32));
	#if (MP_WBITS == 64)
	mpzero(1, sp->length);
	#elif (MP_WBITS == 32)
	mpzero(2, sp->length);
...
{
	register guint32 a, b, c, d, e, f, g, h, temp;
	register guint32 *w;
	register byte t;
	...
	while (t--)
	{
		register guint32 temp = sig1(w[-2]) + w[-7] + sig0(w[-15]) + w[-16];
		*(w++) = temp;
	}
